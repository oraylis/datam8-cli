...
def keys(
    ...
    if apply:
        resolved, _ = read_solution(solution_path)
        with lock_context(opts=opts, lock_file_root=resolved.root_dir):
            result = refactor_keys(solution_path=solution_path, renames=dict(mapping), apply=True)
    else:
        result = refactor_keys(solution_path=solution_path, renames=dict(mapping), apply=False)
...
def values(
    ...
    if apply:
        resolved, _ = read_solution(solution_path)
        with lock_context(opts=opts, lock_file_root=resolved.root_dir):
            result = refactor_values(solution_path=solution_path, old=old, new=new, key=key, apply=True)
    else:
        result = refactor_values(solution_path=solution_path, old=old, new=new, key=key, apply=False)
    payload = {"dryRun": not apply, "result": result}
    emit_result(opts, payload, human_lines=[f"updatedFiles: {result['updatedFiles']} (dryRun={not apply})"])
...
def entity_id(
    ...
    if apply:
        resolved, _ = read_solution(solution_path)
        with lock_context(opts=opts, lock_file_root=resolved.root_dir):
            result = refactor_entity_id(solution_path=solution_path, old=old, new=new, apply=True)
    else:
        result = refactor_entity_id(solution_path=solution_path, old=old, new=new, apply=False)
    payload = {"dryRun": not apply, "result": result}
    emit_result(opts, payload, human_lines=[f"updatedFiles: {result['updatedFiles']} (dryRun={not apply})"])
