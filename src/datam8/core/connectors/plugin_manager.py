...
def _wheelhouse_dir() -> Path | None:
    configured = (os.environ.get("DATAM8_PLUGIN_WHEELHOUSE") or "").strip()
    if not configured:
        return None
...
def sync_bundled_wheels(*, plugin_dir: Path, wheels_dir: Path) -> dict[str, Any]:
    """Install/update bundled connector wheels from a local wheelhouse."""
    ...
    for wheel_file in sorted(wheels_dir.glob("*.whl"), key=lambda p: p.name.lower()):
        ...
        try:
            info = _inspect_wheel_bundle(
                wheel_bytes=wheel_bytes,
                file_name=wheel_file.name,
                strict_connector=False,
            )
        except Exception as e:
            result["errors"][wheel_file.name] = str(e)
            continue
        if info is None:
            continue

        connector_id = info["connectorId"]
        connector_root = _connectors_root(plugin_dir) / connector_id
        distribution = None
        marker_path = connector_root / DISTRIBUTION_MARKER
        if marker_path.exists():
            try:
                distribution = _read_json(marker_path)
            except Exception:
                distribution = None
        current_sha = (
            str(distribution.get("sha256")).strip()
            if isinstance(distribution, dict)
            else ""
        )
        next_sha = _sha256_hex(wheel_bytes)
        if current_sha and current_sha == next_sha:
            result["skipped"].append(connector_id)
            continue

        _install_wheel_from_bytes(
            plugin_dir=plugin_dir,
            wheel_bytes=wheel_bytes,
            file_name=wheel_file.name,
        ...
        result["installed"].append(connector_id)
    return result
