...
class Model:
    ...
    def get_entity_by_locator(self, locator: str | Locator, /) -> EntityWrapperVariant:
        ...
        if self.has_locator(locator):
            wrapper = cast(EntityWrapperVariant, getattr(self, locator.entityType)[locator])
        else:
    ...
    def rename_entity(
        ...
        if from_locator.entityType != to_locator.entityType:
            raise utils.create_error(
                Exception("The entity type of both locators need to be the same when renaming an entity")
            )

        _type = b.EntityType(from_locator.entityType)
        if _type in [b.EntityType.MODEL_ENTITIES, b.EntityType.FOLDERS]:
            raise utils.create_error(
                Exception("Use /entities/move for model entity and folder renames")
            )

        if from_locator.entityName is None or to_locator.entityName is None:
            raise utils.create_error(
                Exception("Rename locators must point to a single entity")
            )

        if self.has_locator(to_locator):
            raise utils.create_error(
                Exception(f"Target of entity rename does already exist: {to_locator}")
            )

        if _type == b.EntityType.PROPERTY_VALUES:
            if len(to_locator.folders) != 1:
                raise utils.create_error(errors.InvalidLocatorError(str(to_locator)))
        elif len(to_locator.folders) != 0:
            raise utils.create_error(errors.InvalidLocatorError(str(to_locator)))

        entity_dict = cast(EntityDict[b.BaseEntityType], getattr(self, from_locator.entityType))
        wrapper = cast(EntityWrapper[b.BaseEntityType], self.get_entity_by_locator(from_locator))
        source_file = wrapper.source_file

        del entity_dict[from_locator]
        ...
        entity_dict[to_locator] = wrapper
        ...
        logger.debug(f"Renamed {from_locator} to {to_locator}")
    ...
    def move_entity(
        ...
        entity_dict = cast(EntityDict[b.BaseEntityType], getattr(self, _from.entityType))

        # create a copy from the old wrapper and mark it for deletion
        from_wrapper = cast(EntityWrapper[b.BaseEntityType], self.get_entity_by_locator(_from))
        ...
        return cast(EntityWrapperVariant, to_wrapper)
    ...
    def delete(self, *, wrappers: list[EntityWrapperVariant]) -> bool:
        ...
        match self._type:
            ...
            case b.EntityType.PROPERTY_VALUES:
                current_content = b.BaseEntities.from_json_file(self.file_path)
                entities = cast(
                    list[b.BaseEntityType], getattr(current_content.root, self._type.value)
                )

                property_wrappers = cast(list[EntityWrapper[p.PropertyValue]], wrappers)
                wrapper_keys = {(w.entity.property, w.entity.name) for w in property_wrappers}
                entities = [
                    e
                    for e in entities
            ...
            case _:
                ...
                for wrapper in wrappers:
                    ...
                    for existing_idx, existing_entity in enumerate(entities):
                        if previous_locator is not None and self._type == b.EntityType.PROPERTY_VALUES:
                            same_key = (
                                existing_entity.name == previous_locator.entityName
                                and getattr(existing_entity, "property", None)
                                == (previous_locator.folders[0] if previous_locator.folders else None)
                            )
                        elif previous_locator is not None:
                            same_key = existing_entity.name == previous_locator.entityName
