...
def test_backend_startup_syncs_bundled_connector_wheels(
    api_client,
    fixture_connector_plugins_dir: Path,
    temp_plugin_dir: Path,
    monkeypatch,
) -> None:
    token = "plugins-bundled-wheel-sync"
    wheel_name = "datam8_plugin_test_conn-0.1.0-py3-none-any.whl"
    wheel_bytes = _build_fixture_wheel(
        fixture_connector_plugins_dir,
        "test-conn",
        wheel_name,
    )
    wheelhouse = temp_plugin_dir / "wheelhouse"
    wheelhouse.mkdir(parents=True, exist_ok=True)
    (wheelhouse / wheel_name).write_bytes(wheel_bytes)

    monkeypatch.setenv("DATAM8_PLUGIN_WHEELHOUSE", str(wheelhouse))
    monkeypatch.setenv("DATAM8_AUTO_INSTALL_BUNDLED_WHEELS", "1")
